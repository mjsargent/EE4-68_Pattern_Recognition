...
%[VSb DSb] = eig(Sb);
    ...
    for Mlda = 50:50
        for Mpca = Mlda:312
            ...
            % percentage
            accuracy(Mpca)= sum(~logical(results))/length(ITesting)*100;
        end
    end
...
set(gca,'FontSize',28);
xlabel('$M_{PCA}$','Interpreter','Latex','fontsize',40);
ylabel('Success Rate ($\%$)','Interpreter','Latex','fontsize',40);
xticks([51,100,150,200,250,300])
xlim([51 312])
print -depsc MpcaVaryMldaConst
