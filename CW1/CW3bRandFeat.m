...
maxSingle = zeros(16,nTrials);
meanSingle = zeros(16,nTrials);
maxMaj = zeros(16,nTrials);
...
% Mpca = 312, Mlda = 51
[Wlda Dlda] = eigs((Vl(:,1:200)'*Sw*Vl(:,1:200))\Vl(:,1:200)'*Sb*Vl(:,1:200),30);      
Wopt = (Wlda'*Vl(:,1:200)')';
...
for nFixed = 0:10:150
    ...
    maxSingle(i,trial) = max(accuracy(1:nSub,1));
    maxMaj(i,trial) = accuracy(nSub+1,1)
    meanSingle(i,trial) = mean(accuracy(1:nSub,1));
    i = i+1;
end
end
...
figure;hold on;plot(0:10:150,mean(maxMaj,2));plot(0:10:150,mean(meanSingle,2));
plot(0:10:150,ones(16,1)*baselineFisherFace);
set(gca,'FontSize',28);
xlim([0 150]);xlabel('$\rho$','Interpreter','Latex','fontsize',40);ylim([35 90])
ylabel('Success Rate ($\%$)','Interpreter','Latex','fontsize',40);
xticks([(0:25:150)]);xticklabels({'1','25','50','75','100','125','150'})
legend('Maj','Models','Baseline','Location','SouthEast','Interpreter','Latex','fontsize',40)
print('effectOfNfixed','-depsc')
